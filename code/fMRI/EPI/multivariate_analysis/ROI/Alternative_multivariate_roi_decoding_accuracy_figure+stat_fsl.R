...
result_dir <- paste0(dir, 'mvpa_2_results_2nd/ROI/')
behavior_result_dir <-("/home/taehyun/Desktop/Behavior_MSHP/data/behavior_fMRI")

#set roi
roi <- c("uni_R", "multi_R_1", "multi_R_2", "uni_D", "multi_D_1", "multi_D_2")
#roi_R <- c("[-27, -4, 53]")
#roi_F <- c("[-39, 14, 29]")
#roi_D <- c("[-45, 35, 17]")
...
for (i in 1:length(sub)) {
  ...
  for (h in 1:length(hier)) {
    data_sub <- read.delim(paste0(sub_dir, hier[h], '/between_alternative_final/ROI/fsl_', hier[h], '_decoding_accuracy_sub', sub[i], '.txt'), sep="")
...
#     ROI == "final_R_ROI" ~ "uni_R",
#     ROI == "final_mvpa_R_ROI_1" ~ "multi_R_1",
#     ROI == "final_mvpa_R_ROI_2" ~ "multi_R_2",
#     ROI == "final_D_ROI" ~ "uni_D",
#     ROI == "final_mvpa_D_ROI_1" ~ "multi_D_1",
#     ROI == "final_mvpa_D_ROI_2" ~ "multi_D_2"
#   ))

roi_total <- data_raw %>%
  group_by(Subject, Hier, ROI) %>%
  summarise(accuracy = mean(accuracy) + 25) %>%
  ungroup() %>%
...
for (r in 1:length(roi)) {
  write_csv(accuracy_posthoc(roi_total, r), paste0(result_dir, roi[r], '/fsl_accuracy_posthoc_in_', roi[r], '.csv'))
}
...
plot_accuracy <- function(res, r) {
  ...
  my_plot_accuracy
  figure_dir <- paste0(result_dir, roi[r], '/')
  ggsave(paste0(figure_dir,"fsl_accuracy_roi_", roi[r], "_final.svg"), width = 120, height = 120, units = "mm")
}

for (r in 1:length(roi)) {
  plot_accuracy(roi_total, r)
}
