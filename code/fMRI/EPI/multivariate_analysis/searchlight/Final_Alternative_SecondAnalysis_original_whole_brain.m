...
for c = 1:length(contrast_folder)
    mkdir(sprintf('%sanalysis/mvpa_2_results_2nd/searchlight/%s/between_alternative_final_ninputdim/whole_brain',project_dir,contrast_folder{c}))
end
...
for i = 1:length(contrast_name)
    for s = 1:length(subjects)
        second_contrasts{i}{s} = strcat(project_dir, 'analysis/', subjects{s}, '/analysis/multivariate_analysis_2/', contrast_folder{i}, '/between_alternative_final_ninputdim/mswres_accuracy_minus_chance_feature.nii,1');
    end
end
...
for i=1:length(contrast_folder)
    contrast_dir{i} = strcat(project_dir, 'analysis/mvpa_2_results_2nd/searchlight/', contrast_folder{i}, '/between_alternative_final_ninputdim/whole_brain');
    Est_file{i} = strcat(project_dir, 'analysis/mvpa_2_results_2nd/searchlight/', contrast_folder{i}, '/between_alternative_final_ninputdim/whole_brain/SPM.mat');
end
