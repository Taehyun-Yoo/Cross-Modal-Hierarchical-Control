function Alternative_LSS_D_block_original_batch(s)
    ...
    for r = 1:run_num
        ...
        % load txt file
            fid = fopen(strcat(project_dir, 'RAW/main/behavioral_data/Onset/', subjects{s}, '/', num2str(r+6), '_D1_', condition_name{c}, '.txt'), 'rt');
    ...
    for r = 1:run_num
        for c = 1:length(condition_name)
        % load txt file
            fid = fopen(strcat(project_dir, 'RAW/main/behavioral_data/Onset/', subjects{s}, '/', num2str(r+6), '_D1_', condition_name{c}, '_RT.txt'), 'rt');
    ...
    for r = 1:run_num
        for c = 1:length(condition_name)
        % load txt file
            fid = fopen(strcat(project_dir, 'RAW/main/behavioral_data/Onset/', subjects{s}, '/', num2str(r+6), '_D1_', condition_name{c}, '_Block.txt'), 'rt');
    ...
    for r = 1:run_num
        fid = fopen(strcat(project_dir, 'RAW/main/behavioral_data/Onset/', subjects{s}, '/', num2str(r+6), '_D1_', '1234.txt'), 'rt');
        onsets_all{r} = cell2mat(textscan(fid, '%f', 'Delimiter', '\n') ) + scan * TR * (r-1);
        fclose(fid);
    end
    onsets_all_hierarchy = vertcat(onsets_all{1}, onsets_all{2});
    ...
    for r = 1:run_num        
        for c = 1:condition_num
            ...
            for k = 1:correct_num_sub_run_con{r}{c}
                if c == 1
                    trial_in{r}{c}{k} = [num2str(r) '_D1_1_' num2str(blocks{r}{c}(k)) '_' num2str(k)];
                elseif c == 2
                    trial_in{r}{c}{k} = [num2str(r) '_D1_2_' num2str(blocks{r}{c}(k)) '_' num2str(k)];
                elseif c == 3
                    trial_in{r}{c}{k} = [num2str(r) '_D1_3_' num2str(blocks{r}{c}(k)) '_' num2str(k)];
                else
                    trial_in{r}{c}{k} = [num2str(r) '_D1_4_' num2str(blocks{r}{c}(k)) '_' num2str(k)];
                end
    ...
    for r = 1:run_num
        for k = 1:correct_num_sub_run{r}
            mkdir(sprintf('%s/analysis/multivariate_analysis_2/D/trial_RT_block_original/%s',subj_dir, trial_total{r}{k}))
        end
    end
    ...
    for r = 1:run_num
        for k = 1:correct_num_sub_run{r}
            if exist(strcat(subj_dir, '/analysis/multivariate_analysis_2/D/trial_RT_block_original/', trial_total{r}{k}, '/SPM.mat'), 'file') == 2
                delete(strcat(subj_dir, '/analysis/multivariate_analysis_2/D/trial_RT_block_original/', trial_total{r}{k}, '/*'))
            end
        end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    ...
    for r = 1:run_num
        for c = 1:correct_num_sub_run{r}
            ...
            matlabbatch{1}.spm.stats.fmri_spec.dir = {strcat(subj_dir, '/analysis/multivariate_analysis_2/D/trial_RT_block_original/', trial_total{r}{c})};
            ...
            run_dir = fullfile(subj_dir, 'functional_concat', 'D', 'all');
            ...
            if r == 1
                tmp = RTs_total{1};
                tmp(c) = [];
                matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).duration = vertcat(tmp, RTs_total{2});
            else
                tmp = RTs_total{2};
                tmp(c) = [];
                matlabbatch{1}.spm.stats.fmri_spec.sess.cond(2).duration = vertcat(RTs_total{1}, tmp);
            end
            ...
            spm_fmri_concatenate(strcat(subj_dir, '/analysis/multivariate_analysis_2/D/trial_RT_block_original/', trial_total{r}{c}, '/SPM.mat'), scans);

        %% First level model estimation
            matlabbatch = {};

            matlabbatch{1}.spm.stats.fmri_est.spmmat = {strcat(subj_dir, '/analysis/multivariate_analysis_2/D/trial_RT_block_original/', trial_total{r}{c}, '/SPM.mat')};
