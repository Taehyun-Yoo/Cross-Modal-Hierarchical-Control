function Alternative_LSS_R_block_original_batch(s)
    ...
    for r = 1:run_num
        ...
        % load txt file
            fid = fopen(strcat(project_dir, 'RAW/main/behavioral_data/Onset/', subjects{s}, '/', num2str(r), '_R1_', condition_name{c}, '.txt'), 'rt');
    ...
    for r = 1:run_num
        for c = 1:length(condition_name)
        % load txt file
            fid = fopen(strcat(project_dir, 'RAW/main/behavioral_data/Onset/', subjects{s}, '/', num2str(r), '_R1_', condition_name{c}, '_RT.txt'), 'rt');
    ...
    for r = 1:run_num
        for c = 1:length(condition_name)
        % load txt file
            fid = fopen(strcat(project_dir, 'RAW/main/behavioral_data/Onset/', subjects{s}, '/', num2str(r), '_R1_', condition_name{c}, '_Block.txt'), 'rt');
    ...
    for r = 1:run_num
        fid = fopen(strcat(project_dir, 'RAW/main/behavioral_data/Onset/', subjects{s}, '/', num2str(r), '_R1_', '1234.txt'), 'rt');
    ...
    for r = 1:run_num        
        for c = 1:condition_num
            ...
            for k = 1:correct_num_sub_run_con{r}{c}
                if c == 1
                    trial_in{r}{c}{k} = [num2str(r) '_R1_1_' num2str(blocks{r}{c}(k)) '_' num2str(k)];
                elseif c == 2
                    trial_in{r}{c}{k} = [num2str(r) '_R1_2_' num2str(blocks{r}{c}(k)) '_' num2str(k)];
                elseif c == 3
                    trial_in{r}{c}{k} = [num2str(r) '_R1_3_' num2str(blocks{r}{c}(k)) '_' num2str(k)];
                else
                    trial_in{r}{c}{k} = [num2str(r) '_R1_4_' num2str(blocks{r}{c}(k)) '_' num2str(k)];
                end
    ...
    for r = 1:run_num
        for k = 1:correct_num_sub_run{r}
            mkdir(sprintf('%s/analysis/multivariate_analysis_2/R/trial_RT_block_original/%s',subj_dir, trial_total{r}{k}))
        end
    end
    ...
    for r = 1:run_num
        for k = 1:correct_num_sub_run{r}
            if exist(strcat(subj_dir, '/analysis/multivariate_analysis_2/R/trial_RT_block_original/', trial_total{r}{k}, '/SPM.mat'), 'file') == 2
                delete(strcat(subj_dir, '/analysis/multivariate_analysis_2/R/trial_RT_block_original/', trial_total{r}{k}, '/*'))
            end
        end
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    ...
    for r = 1:run_num
        for c = 1:correct_num_sub_run{r}
            ...
            matlabbatch{1}.spm.stats.fmri_spec.dir = {strcat(subj_dir, '/analysis/multivariate_analysis_2/R/trial_RT_block_original/', trial_total{r}{c})};
            ...
            run_dir = fullfile(subj_dir, 'functional_concat', 'R', 'all');
            ...
            spm_fmri_concatenate(strcat(subj_dir, '/analysis/multivariate_analysis_2/R/trial_RT_block_original/', trial_total{r}{c}, '/SPM.mat'), scans);

        %% First level model estimation
            matlabbatch = {};

            matlabbatch{1}.spm.stats.fmri_est.spmmat = {strcat(subj_dir, '/analysis/multivariate_analysis_2/R/trial_RT_block_original/', trial_total{r}{c}, '/SPM.mat')};
