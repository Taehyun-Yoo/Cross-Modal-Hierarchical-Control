...
for s = 1:length(sub_number)
    ...
    if exist(sprintf('%s/ROI_analysis/ROI_2voxels/D_parametric',sub_dir), 'dir') ~= 7
        mkdir(sprintf('%s/ROI_analysis/ROI_2voxels/D_parametric',sub_dir));
    end

    % open file
    fi = fopen(fullfile(sub_dir, 'ROI_analysis', 'ROI_2voxels', 'D_parametric', sprintf('D_parametric_beta_sub%02d.txt', sub_number(s))), 'w');

    % first load design for each participant
    D = mardo(fullfile(sub_dir, 'parametric_analysis', 'D_parametric', 'SPM.mat'));
