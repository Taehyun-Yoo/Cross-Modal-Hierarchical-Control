...
for s = 1:length(sub_number)
    ...
    if exist(sprintf('%s/ROI_analysis_2/ROI_2voxels/F_parametric',sub_dir), 'dir') ~= 7
        mkdir(sprintf('%s/ROI_analysis_2/ROI_2voxels/F_parametric',sub_dir));
    end

    % open file
    fi = fopen(fullfile(sub_dir, 'ROI_analysis_2', 'ROI_2voxels', 'F_parametric', sprintf('F_parametric_beta_sub%02d.txt', sub_number(s))), 'w');

    % first load design for each participant
    D = mardo(fullfile(sub_dir, 'parametric_analysis_2_-inf', 'F_parametric', 'SPM.mat'));
