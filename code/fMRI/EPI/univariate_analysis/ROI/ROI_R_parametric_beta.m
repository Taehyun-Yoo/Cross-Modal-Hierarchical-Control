...
roi_dir = strcat(project_dir, 'Code/EPI/univariate_analysis/ROI/Prerequisite_2/'); %  fullfile(subjroot, 'rois');
...
condition_name = {'increasing_complexity_R'};    % name of conditions
...
for s = 1:length(sub_number)
    ...
    if exist(sprintf('%s/ROI_analysis_2',sub_dir), 'dir') ~= 7
        mkdir(sprintf('%s/ROI_analysis_2',sub_dir));
    end
    
    if exist(sprintf('%s/ROI_analysis_2/ROI_2voxels',sub_dir), 'dir') ~= 7
        mkdir(sprintf('%s/ROI_analysis_2/ROI_2voxels',sub_dir));
    end

    if exist(sprintf('%s/ROI_analysis_2/ROI_2voxels/R_parametric',sub_dir), 'dir') ~= 7
        mkdir(sprintf('%s/ROI_analysis_2/ROI_2voxels/R_parametric',sub_dir));
    end

    % open file
    fi = fopen(fullfile(sub_dir, 'ROI_analysis_2', 'ROI_2voxels', 'R_parametric', sprintf('R_parametric_beta_sub%02d.txt', sub_number(s))), 'w');

    % first load design for each participant
    D = mardo(fullfile(sub_dir, 'parametric_analysis_2_-inf', 'R_parametric', 'SPM.mat'));
