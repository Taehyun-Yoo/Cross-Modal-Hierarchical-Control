...
mkdir(sprintf('%sanalysis/results_2nd/parametric_analysis_2_-inf/F_parametric_whole_brain',project_dir))
for c = 1:length(contrast_folder)
    mkdir(sprintf('%sanalysis/results_2nd/parametric_analysis_2_-inf/F_parametric_whole_brain/%s',project_dir,contrast_folder{c}))
end
...
for i = 1:length(contrast_name)
    for s = 1:length(subjects)
        second_contrasts{i}{s} = strcat(project_dir, 'analysis/', subjects{s}, '/analysis/parametric_analysis_2_-inf/F_parametric/con_000', num2str(i), '.nii,1');
    end
end
...
for i=1:length(contrast_folder)
    contrast_dir{i} = strcat(project_dir, 'analysis/results_2nd/parametric_analysis_2_-inf/F_parametric_whole_brain/', contrast_folder{i});
    Est_file{i} = strcat(project_dir, 'analysis/results_2nd/parametric_analysis_2_-inf/F_parametric_whole_brain/', contrast_folder{i},'/SPM.mat');
end
