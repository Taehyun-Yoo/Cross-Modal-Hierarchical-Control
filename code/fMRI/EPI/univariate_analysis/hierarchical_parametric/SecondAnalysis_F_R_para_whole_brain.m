...
mkdir(sprintf('%sanalysis/results_2nd/parametric_analysis_2_-inf/F_R_parametric_whole_brain',project_dir))
for c = 1:length(contrast_folder)
    mkdir(sprintf('%sanalysis/results_2nd/parametric_analysis_2_-inf/F_R_parametric_whole_brain/%s',project_dir,contrast_folder{c}))
end
...
for i=1:length(contrast_folder)
    contrast_dir{i} = strcat(project_dir, 'analysis/results_2nd/parametric_analysis_2_-inf/F_R_parametric_whole_brain/', contrast_folder{i});
    Est_file{i} = strcat(project_dir, 'analysis/results_2nd/parametric_analysis_2_-inf/F_R_parametric_whole_brain/', contrast_folder{i},'/SPM.mat');
end
...
for i=1:length(contrast_folder)
    ...
    for s = 1:length(subjects)
        % F 대비 경로
        F_path = strcat(project_dir, 'analysis/', subjects{s}, '/analysis/parametric_analysis_2_-inf/F_parametric/con_000', num2str(i), '.nii');
        % R 대비 경로
    ...
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.name    = 'F > R';
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.weights = [1 -1];
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep = 'none';
    
    % (옵션) R > F
    matlabbatch{3}.spm.stats.con.consess{2}.tcon.name    = 'R > F';
    matlabbatch{3}.spm.stats.con.consess{2}.tcon.weights = [-1 1];
    matlabbatch{3}.spm.stats.con.consess{2}.tcon.sessrep = 'none';
    
    % (옵션) 각 조건의 >0
    % matlabbatch{3}.spm.stats.con.consess{3}.tcon.name    = 'F > 0';
    % matlabbatch{3}.spm.stats.con.consess{3}.tcon.weights = [1 0];
    % matlabbatch{3}.spm.stats.con.consess{3}.tcon.sessrep = 'none';
    % 
    % matlabbatch{3}.spm.stats.con.consess{4}.tcon.name    = 'R > 0';
