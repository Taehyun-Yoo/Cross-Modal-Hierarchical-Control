...
mkdir(sprintf('%sanalysis/results_2nd/parametric_analysis_2_-inf/D_F_parametric_whole_brain',project_dir))
for c = 1:length(contrast_folder)
    mkdir(sprintf('%sanalysis/results_2nd/parametric_analysis_2_-inf/D_F_parametric_whole_brain/%s',project_dir,contrast_folder{c}))
end
...
for i=1:length(contrast_folder)
    contrast_dir{i} = strcat(project_dir, 'analysis/results_2nd/parametric_analysis_2_-inf/D_F_parametric_whole_brain/', contrast_folder{i});
    Est_file{i} = strcat(project_dir, 'analysis/results_2nd/parametric_analysis_2_-inf/D_F_parametric_whole_brain/', contrast_folder{i},'/SPM.mat');
end
...
for i=1:length(contrast_folder)
    ...
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.name    = 'D > F';
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.weights = [1 -1];
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep = 'none';
    
    % (옵션) R > F
    matlabbatch{3}.spm.stats.con.consess{2}.tcon.name    = 'F > D';
