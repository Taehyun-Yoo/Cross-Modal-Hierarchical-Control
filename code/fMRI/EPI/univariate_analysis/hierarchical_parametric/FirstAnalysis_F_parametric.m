...
for s = 1:length(subjects) 
    for r = 1:run_num
        fid_para = fopen(strcat(project_dir, 'RAW/main/behavioral_data/Onset/', subjects{s}, '/', num2str(r+2), '_task.txt'), 'rt');
...
cont_w = [zeros(1,1), 1/4, zeros(1,6), zeros(1,1), 1/4, zeros(1,6), zeros(1,1), 1/4, zeros(1,6), zeros(1,1), 1/4, zeros(1,6), zeros(1,4)];
...
for s = 1:1
    ...
    if exist(strcat(subj_dir, '/analysis/parametric_analysis_2_-inf/F_parametric/SPM.mat'), 'file') == 2
       delete(strcat(subj_dir, '/analysis/parametric_analysis_2_-inf/F_parametric/*'))
    end
    
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    
        
    matlabbatch{1}.spm.stats.fmri_spec.dir = {fullfile(subj_dir, 'analysis/parametric_analysis_2_-inf/F_parametric')};
    ...
    for r = 1:run_num
        ...
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond.duration = F_duration;
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond.tmod = 0;
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond.pmod.name = 'complexity';
        matlabbatch{1}.spm.stats.fmri_spec.sess(r).cond.pmod.param = [1; 1; 2; 2; 4; 4] - mean([1; 1; 2; 2; 4; 4]);
    ...
    matlabbatch{2}.spm.stats.fmri_est.spmmat = {fullfile(subj_dir, 'analysis/parametric_analysis_2_-inf/F_parametric', 'SPM.mat')};
    ...
    matlabbatch{3}.spm.stats.con.spmmat = {fullfile(subj_dir, 'analysis/parametric_analysis_2_-inf/F_parametric', 'SPM.mat')};
