...
result_dir <- paste0(dir, 'results_2nd/only_y_2/')
...
R <- subset(H_contrast$y_coor, H_contrast$Hierarchy == 1)
F <- subset(H_contrast$y_coor, H_contrast$Hierarchy == 2)
D <- subset(H_contrast$y_coor, H_contrast$Hierarchy == 3)
...
y_one_way_ANOVA <- function(res) {
  one_way_ANOVA <- anova_test(data = res, dv = y_coor, wid = Subject, within = Hierarchy, effect.size = "pes")
...
write_csv(y_one_way_ANOVA(my_data), paste0(result_dir, 'one_way_ANOVA.csv'))

y_posthoc <- function(res) {
  post_t_test <- t_test(y_coor ~ Hierarchy, p.adjust.method = "holm", data = res, paired = TRUE, detailed = TRUE)
  .y. <- c("y_coor", "y_coor", "y_coor")
...
plot_y <- function(res) {
  ...
  if (nrow(total_K_y(res)) != 0){
    my_plot_y <- my_plot_y +
      add_pvalue(total_K_y(res),
  ...
  # ggsave(paste0(result_dir,"Y_final.svg"), width = 120, height = 120, units = "mm")
  ggsave(paste0(result_dir,"Y_final_no_tick.svg"), width = 120, height = 120, units = "mm")
}
