...
result_dir <- paste0(dir, 'results_2nd/only_x_2/')

H_contrast <- read.table(paste0(result_dir, 'total_data_left_0_05_k0.txt'), quote="\"", comment.char="")

names(H_contrast)[1] <-c("x_coor")
names(H_contrast)[2] <-c("y_coor")
names(H_contrast)[3] <-c("z_coor")
names(H_contrast)[4] <-c("Hierarchy")
names(H_contrast)[5] <-c("Directionality")
names(H_contrast)[6] <-c("Subject")

k <- c()
for (i in 1:dim(H_contrast)[1]){
  k[i] = sum(H_contrast[i,6] == H_contrast[,6]);
}
...
H_contrast <- cbind(H_contrast, index);
H_contrast <- subset(H_contrast, H_contrast$index == 1) %>%
  select(-index);
...
R <- subset(H_contrast$x_coor, H_contrast$Hierarchy == 1)
F <- subset(H_contrast$x_coor, H_contrast$Hierarchy == 2)
D <- subset(H_contrast$x_coor, H_contrast$Hierarchy == 3)
...
hier <- my_data %>%
  group_by(Hierarchy) %>%
  summarise(se = sd(x_coor)/sqrt(n()),
            x_coor = mean(x_coor)) %>%
  ungroup()

hier$Hierarchy <- factor(hier$Hierarchy, levels = c("R", "F", "D"))

#One_way_ANOVA_SVD (each hierarchy)
x_one_way_ANOVA <- function(res) {
  one_way_ANOVA <- anova_test(data = res, dv = x_coor, wid = Subject, within = Hierarchy, effect.size = "pes")
...
write_csv(x_one_way_ANOVA(my_data), paste0(result_dir, 'one_way_ANOVA.csv'))

x_posthoc <- function(res) {
  post_t_test <- t_test(x_coor ~ Hierarchy, p.adjust.method = "holm", data = res, paired = TRUE, detailed = TRUE)
  .y. <- c("z_coor", "z_coor", "z_coor")
...
plot_x <- function(res) {
  ...
  my_plot_x <- ggplot(x_hier,aes(x=Hierarchy,y=x_coor)) +
    ...
    scale_y_reverse(breaks = seq(-40, 0, 10), limits = c(0, -45))+
  ...
  if (nrow(total_K_x(res)) != 0){
    my_plot_x <- my_plot_x +
      add_pvalue(total_K_x(res),
  ...
  #ggsave(paste0(result_dir,"X_final.svg"), width = 120, height = 120, units = "mm")
  ggsave(paste0(result_dir,"X_final_no_tick.svg"), width = 120, height = 120, units = "mm")
}
