...
result_dir <- paste0(dir, 'results_2nd/only_z_2/')
...
R <- subset(H_contrast$z_coor, H_contrast$Hierarchy == 1)
F <- subset(H_contrast$z_coor, H_contrast$Hierarchy == 2)
D <- subset(H_contrast$z_coor, H_contrast$Hierarchy == 3)
...
z_one_way_ANOVA <- function(res) {
  one_way_ANOVA <- anova_test(data = res, dv = z_coor, wid = Subject, within = Hierarchy, effect.size = "pes")
...
write_csv(z_one_way_ANOVA(my_data), paste0(result_dir, 'one_way_ANOVA.csv'))

z_posthoc <- function(res) {
  post_t_test <- t_test(z_coor ~ Hierarchy, p.adjust.method = "holm", data = res, paired = TRUE, detailed = TRUE)
...
write_csv(z_posthoc(my_data), paste0(result_dir, 'one_way_posthoc.csv'))
...
plot_z <- function(res) {
  ...
  if (nrow(total_K_z(res)) != 0){
    my_plot_z <- my_plot_z +
      add_pvalue(total_K_z(res),
  ...
  #ggsave(paste0(result_dir,"Z_final.svg"), width = 120, height = 120, units = "mm")
  ggsave(paste0(result_dir,"Z_final_no_tick.svg"), width = 120, height = 120, units = "mm")
}
